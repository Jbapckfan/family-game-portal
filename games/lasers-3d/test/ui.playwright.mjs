...
async function run() {
  ...
  try {
    ...
    for (const vp of VIEWPORTS) {
      ...
      const tiles = await page.evaluate(() => ({ n: document.querySelectorAll('.level-tile').length, locked: document.querySelectorAll('.level-tile[data-state="locked"]').length, five: document.querySelector('.level-tile[data-index="4"]').getAttribute('data-state') }));
      assert(tiles.n === 20 && tiles.locked === 15 && tiles.five !== 'locked', `level select: 20 tiles, 15 locked, level 5 open ${JSON.stringify(tiles)}`);
